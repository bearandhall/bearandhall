...
export default function EdgeNav() {
  ...
  const guroRef  = useRef<HTMLAnchorElement>(null);
  const techRef  = useRef<HTMLAnchorElement>(null);
  const horseRef = useRef<HTMLAnchorElement>(null);
  const infoRef  = useRef<HTMLAnchorElement>(null);

  // ✅ Record에 RefObject<HTMLAnchorElement>로 매핑
  const refs: Record<Key, RefObject<HTMLAnchorElement>> = {
    home: homeRef,
    guro: guroRef,
    techme: techRef,
    horse: horseRef,
  ...
  return (
    <>
      ...
      {overlay && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/40 backdrop-blur-sm lg:hidden"
             onClick={()=>setOverlay(null)}>
          <div className="pointer-events-auto p-3" onClick={(e)=>e.stopPropagation()}>
            <img src={pageImg[overlay]}
                 onError={(e)=>{ (e.currentTarget as HTMLImageElement).src='/img/pages/placeholder.png'; }}
                 alt={overlay} className="w-[88vw] max-w-[720px] h-auto object-contain rounded" />
          </div>
        </div>
      )}
    </>
